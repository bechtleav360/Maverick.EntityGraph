...
public class MergeDuplicatesJobOld implements ScheduledJob {
    ...
    public MergeDuplicatesJobOld(EntityServices service, QueryServices queryServices, ValueServices valueServices) {
  ...
  ?thing 	<http://schema.org/identifier> "_a1238" .
    ?thing 	<http://schema.org/dateCreated> ?date .
        ...
        SELECT ?name ?type (GROUP_CONCAT(DISTINCT ?entity; separator=", ") AS ?duplicates)
        WHERE {
    ...
    private Flux<DuplicateCandidate> findCandidates(IRI sharedProperty, SessionContext ctx) {
        ...
        } GROUP BY ?shared ?type
        having (count(?entity) > 1)


        PREFIX schema: <http://schema.org/>

        SELECT ?type ?value (GROUP_CONCAT(DISTINCT ?entity; separator=", ") AS ?duplicates)
        WHERE {
            ?entity a ?type ;
                schema:name ?value .
        }
        ...
        Variable sharedValue = SparqlBuilder.var("shared");
